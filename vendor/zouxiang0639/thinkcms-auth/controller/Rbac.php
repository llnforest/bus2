...
class Rbac
{
    ...
    public function menu(){
        ...
        foreach ($result as $n=> $r) {
            ...
            $result[$n]['str_manage'] .= checkPath('auth/menuDelete',["id" => $r['id']]) ?'<a class="span-post" post-msg="你确定要删除吗" post-url="'.url("auth/menuDelete",["id" => $r['id']]).'">删除</a>':'';
            $result[$n]['status'] = $r['status'] ? '开启' : '隐藏';
        }
        $str = "<tr id='node-\$id' \$parent_id_node>
                    <td>
                        <input type='text'  value='\$sort' post-url='".url("auth/menuOrder")."' post-id='\$id' class='change-data form-control'>
                    </td>
                    <td>\$id</td>
                    <td>\$spacer  \$name</td>
                    <td>\$app</td>
    ...
    public function menuAdd(){
        ...
        if($this->request->isPost()){
            ...
            if($menu->menuAdd($post)){
                return ['code'=>1,'msg'=>'增加成功','url'=>url('auth/menu'),'alert'=>'是否返回'];
            }else{
    ...
    public function menuOrder(){
        if($this->request->isPost()) {
            $order  = isset($this->param['data'])?intval($this->param['data']):'';
            $result = Menu::get($this->id);
    ...
    public function role(){
        $data = AuthRole::all(['system_id'=>['in','0,'.Auth::sessionGet('user.system_id')]]);
    ...
    public function roleAdd(){
        ...
        if($this->request->isPost()){
            $post   = $this->post;
            $post['system_id'] = Auth::sessionGet('user.system_id');
            //现在数据
            $validate = new Validate($this->roleValidate);
            if (!$validate->check($post)) {
                return ['code'=>0,'msg'=>$validate->getError()];
    ...
    public function roleStatus(){
        if($this->request->isPost()){
            ...
            $ratify = AuthRole::update(['status'=>$param],['id'=>$id]);
            if($ratify){
    ...
    public function authorize(){
        ...
        $menu       = Menu::where(['id'=>['not in','8,163']])->order(["sort" => "asc",'id'=>'asc'])->column('*','id');
        ...
        if($this->request->isPost()){//表单处理
            ...
            if(!isset($post['menuid'])) return ['code'=>1,'msg'=>'修改成功'];
            $menuid = $post['menuid'];
    ...
    public function adminAuthorize(){
        ...
        //管理员所有角色权限
        $roleId     = AuthRoleUser::hasWhere('authRule')->where(['AuthRoleUser.user_id'=>$this->id,'AuthRole.status'=>1])->column('role_id');
    ...
    public function log(){
        $param  = $this->param;
        $param['system_id'] = Auth::sessionGet('user.system_id');
        $where  = getWhereParam(['user_name','user_id','title'=>'like','create_time'=>['start','end']],$param,0);
        $list   = ActionLog::where($where)->order('id desc')->paginate(50,'',[
            'query'=>$param
        ]);
        $page   = $list->render();
